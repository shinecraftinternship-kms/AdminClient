...
class ApproveClientView(APIView):
    def post(self, request):
        ...
        ActivityLog.objects.create(
            action="approve", client=client, company=company,
            details=f"Client with key {key} approved by {request.user.username}",
        )
...
class ApproveMultipleView(APIView):
    def post(self, request):
        ...
        for client in clients:
            ...
            ActivityLog.objects.create(
                action="approve", client=client, company=company,
                details=f"Client with key {client.registration_key} approved by {request.user.username}",
            )
...
class ConnectionSettingsView(APIView):
    """GET  -> return stored URL + token (never auto-generates)
    POST -> explicitly generate new URL + token
    PUT  -> regenerate token only
    """
